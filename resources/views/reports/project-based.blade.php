...
<div id="page-wrapper">
    <div class="container-fluid">
        ...
        <div class="row report-filter-container">
            <div class="pull-left">
                <form id="project-based-report-form" method="POST" action="{{route('reports.project.based.fetch')}}" class="project-based-report-form form-inline" autocomplete="off">
                <div class="form-group">
                    <div class="col-md-12">
                        <input id="report-project-search" name="report_project_search" placeholder="Search ProjectName, ProjectCode" type="text" class="form-control"/>
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-md-12">
                        <div id="reportrange" title="Select a Range" class="pull-right" style="background: #fff; cursor: pointer; padding: 5px 10px; border: 1px solid #ccc; width: 100%">
                ...
                <div class="form-group">
                    <div class="col-md-12">
                        ...
                        <a id="report-export" data-href="{{route('reports.project.based.export')}}" class="btn btn-default"><span class="glyphicon glyphicon-download"></span> Export</a>
                    </div>
                </div>
                <input id="report-start-date" type="hidden" name="report_start_date"/>
                <input id="report-end-date" type="hidden" name="report_end_date"/>
...
$(document).ready(function(){
    ...
    $('body').on('click','#report-export',function(){
        var parameters = $('#project-based-report-form').serialize();
    ...
    $('body').on('submit','#project-based-report-form',function(e){
        e.preventDefault();
        return false;
    });
    $('#report-project-search').keyup(function(){delayFunction(function(){
            fetchReport();
        }, 800 );
    });
    $('#reportrange').daterangepicker({
        ...
        $('#report-start-date').val(start.format('YYYY-MM-DD'));
        $('#report-end-date').val(end.format('YYYY-MM-DD'));
...
function fetchReport(options) {
    ...
    var filterForm = $('#project-based-report-form');
...
function resetReportFilter(){
    $('#reportrange span').html($('#reportrange').attr('title'));
    $('#report-project-search').val('');
    $('#report-start-date').val('');
    $('#report-end-date').val('');
